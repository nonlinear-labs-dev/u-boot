...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"loaddts=load usb 0:0 ${fdtaddr} ${IMG_NAME} ${DTS_SIZE} ${DTS_START}\0" \
        "nlboot=run nlinit; echo '#### Checking Devices and Partitions ####'; mmc list; run tryusb;\0" \
//	"nlboot= run nlinit; run tryusb; echo '#### Trying new uboot ####'; run trymmc; run trymmc2; run tryemmc; run tryemmc2\0" \
...
//	"tryemmc=mmc dev 1; echo '#### Trying eMMC Boot ####'; if mmc rescan; then load mmc 1:1 ${kloadaddr} boot/uImage; load mmc 1:1 ${fdtaddr} boot/nonlinear-labs-2D.dtb; setenv mmcroot /dev/mmcblk1p1 ro; setenv mmcrootfstype ext4 rootwait; setenv bootargs console=${console} ${optargs} root=${mmcroot} rootfstype=${mmcrootfstype}; bootm ${kloadaddr} - ${fdtaddr}; fi\0" \
//	"tryemmc2=mmc dev 1; echo '#### Trying eMMC2 Boot ####'; if mmc rescan; then load mmc 1:2 ${kloadaddr} boot/uImage; load mmc 1:2 ${fdtaddr} boot/nonlinear-labs-2D.dtb; setenv mmcroot /dev/mmcblk1p2 ro; setenv mmcrootfstype ext4 rootwait; setenv bootargs console=${console} ${optargs} root=${mmcroot} rootfstype=${mmcrootfstype}; bootm ${kloadaddr} - ${fdtaddr}; fi\0" \
